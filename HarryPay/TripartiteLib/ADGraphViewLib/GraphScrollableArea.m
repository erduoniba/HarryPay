...
#define VISIBLE_FRAME CGRectMake(0, 0, 410, 132)
#define GRAPH_SCROLLABLE_VIEW_FRAME_WIDTH_DELTA 28

#define Y_OFFSET_FOR_GRAPH_POINTS 0
...
#define DAY_STRING_FRAME CGRectMake(0, 163, 14, 14)
...
- (CGPoint)pointForValue:(NSNumber *)value atDayNumber:(float)dayNumber{
    ...
    float y = VISIBLE_FRAME.size.height - Y_OFFSET_FOR_GRAPH_POINTS - ([value floatValue]/_averagePoint + MAXIMUM_GRAPH_Y_VALUE) * VALUE_Y_INTERVAL;
    if(y < 10){
        y = 10;
    }
...
- (void)drawLineWithPointsArray:(NSArray *)pointsArray{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(getPoints:)]) {
        [self.delegate getPoints:pointsArray];
    }
    
    self.objectsLine = [[GraphLine alloc] initWithFrame:self.bounds pointsArray:pointsArray minY:[self pointForValue:@(100) atDayNumber:0].y maxY:[self pointForValue:@(300) atDayNumber:0].y];
...
- (void)reloadWithStartDay:(NSInteger)startDay completionCallback:(void(^)(void))callback{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for(int dayNumber = (int)startDay; dayNumber <= numberOfVisibleDays; dayNumber++){
...
- (CGRect)frameForCurrentZoomRate{
    ...
    frame.size.width = newSize.width;   //tag by harry 曲线宽度设置为自由宽度 (newSize.width <= MAXIMUM_FRAME_WIDTH) ? newSize.width : MAXIMUM_FRAME_WIDTH;
...
- (NSInteger)numberOfDaysInWidth:(CGFloat)width{
    ...
    NSInteger numberOfDays = self.numberOfDays;   //tag by harry (每一个点表示一天) (int)(width / self.dayIntervalWidth + 0.5);
...
- (void)drawDateForDayNumber:(NSInteger)dayNumber{
    ...
    NSString *dayString = [NSString stringWithFormat: @"%@%d", (day < 10) ? @"0" : @"", (int)day];
    ...
    [dayString drawInRect:dayStringFrame withAttributes:@{NSFontAttributeName : [UIFont defaultGraphBoldFontWithSize: 12.]}];
    ...
    [month drawAtPoint:CGPointMake(x - MONTH_STRING_FRAME.size.width / 2, MONTH_STRING_FRAME.origin.y) withAttributes:@{NSFontAttributeName : [UIFont defaultGraphBoldFontWithSize: 10.]}];
